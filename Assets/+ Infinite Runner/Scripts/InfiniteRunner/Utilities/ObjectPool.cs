...
public static class ObjectPool
{
    ...
    public static int GetActiveInPool<T>() where T : Component
    {
        List<Component> poolableObjects;
        int count = 0;

        if (Pool.TryGetValue(typeof(T), out poolableObjects))
        {
            foreach (var poolObj in poolableObjects)
            {
    ...
    public static void Remove<T>(T pooledObj) where T : Component
    {
        
        var key = pooledObj.GetType();
        List<Component> pooledObjList;
        if(Pool.TryGetValue(key, out pooledObjList))
            pooledObjList.Remove(pooledObj);
    }
}