...
{
    [SerializeField] GameObject[] flowersPrefab;
    ...
    private void Start()
    {
        ...
        {
            var flower = Instantiate(flowersPrefab[Random.Range(0, flowersPrefab.Length)], transform);
            float yPos = Random.Range(0.75f, 1f);
            flower.transform.localPosition = new Vector2(Random.Range(-0.25f, 0.25f), yPos);
            int scale = Random.Range(0, 1.0f) < 0.5f ? -1 : 1;
            flower.transform.localScale = new Vector2(scale, 1) * Random.Range(1.25f, 1.75f);
        }
